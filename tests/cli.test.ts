...
describe("agent memory", () => {
  describe("remember + recall", () => {
    it("stores and retrieves an observation", () => {
      const id = remember({
        ...
        type: "observation",
        artifactPath: "src/lib/auth.ts",
      });
      expect(id).toBeGreaterThan(0);

      const obs = getObservation(id);
      expect(obs).not.toBeNull();
      expect(obs!.agentId).toBe("claude-code");
      expect(obs!.content).toContain("JWT tokens");
      expect(obs!.type).toBe("observation");
      expect(obs!.artifactPath).toBe("src/lib/auth.ts");
      expect(obs!.confidence).toBe(1.0);
    });

    it("stores different observation types", () => {
      const types = ["observation", "question", "insight", "decision", "context"] as const;
      for (const type of types) {
        const id = remember({ content: `Test ${type}`, type });
        expect(id).toBeGreaterThan(0);
        expect(getObservation(id)!.type).toBe(type);
      }
    });

    it("defaults agentId and sessionId", () => {
      const id = remember({ content: "Minimal observation" });
      const obs = getObservation(id);
      expect(obs!.agentId).toBe("unknown");
      expect(obs!.sessionId).toBe("default");
    });

    it("stores custom confidence", () => {
      const id = remember({ content: "Low confidence guess", confidence: 0.3 });
      expect(getObservation(id)!.confidence).toBeCloseTo(0.3);
    });
  });

  describe("recall with filters", () => {
    beforeEach(() => {
      const session = `recall-test-${Date.now()}`;
      remember({ agentId: "claude", sessionId: session, content: "Auth uses JWT", type: "observation" });
      remember({ agentId: "claude", sessionId: session, content: "Should we use OAuth2?", type: "question" });
      remember({ agentId: "cursor", sessionId: session, content: "Config is in hub.config.ts", type: "insight" });
    });

    it("recalls by search keyword", () => {
      const results = recall({ search: "JWT" });
      expect(results.length).toBeGreaterThanOrEqual(1);
      expect(results[0].content).toContain("JWT");
    });

    it("recalls by type", () => {
      const questions = recall({ type: "question" });
      for (const q of questions) expect(q.type).toBe("question");
    });

    it("recalls by agentId", () => {
      const cursorObs = recall({ agentId: "cursor" });
      for (const o of cursorObs) expect(o.agentId).toBe("cursor");
    });

    it("respects limit", () => {
      const results = recall({ limit: 2 });
      expect(results.length).toBeLessThanOrEqual(2);
    });

    it("returns most recent first", () => {
      const results = recall({ limit: 5 });
      if (results.length >= 2) {
  ...
  describe("getObservationCounts", () => {
    it("returns counts by type", () => {
      remember({ content: "count test", type: "insight" });
      const counts = getObservationCounts();
      expect(typeof counts).toBe("object");
      // At least one type should have a count
  ...
  describe("getKnownAgents", () => {
    it("returns agents that have written observations", () => {
      remember({ agentId: `agent-known-${Date.now()}`, content: "test" });
  ...
  describe("getAgentSessions", () => {
    it("returns sessions for an agent", () => {
      const agent = `agent-sessions-${Date.now()}`;
      remember({ agentId: agent, sessionId: "s1", content: "first" });
      remember({ agentId: agent, sessionId: "s2", content: "second" });
      const sessions = getAgentSessions(agent);
      expect(sessions.length).toBeGreaterThanOrEqual(2);
    });
  });

  describe("forget", () => {
    it("deletes a single observation", () => {
      const id = remember({ content: "to forget" });
      expect(forgetObservation(id)).toBe(true);
      expect(getObservation(id)).toBeNull();
    });

    it("deletes all observations in a session", () => {
      const session = `forget-session-${Date.now()}`;
      remember({ sessionId: session, content: "a" });
      remember({ sessionId: session, content: "b" });
      const count = forgetSession(session);
      expect(count).toBeGreaterThanOrEqual(2);
    });

    it("returns false for non-existent observation", () => {
      expect(forgetObservation(999999)).toBe(false);
    });
  });

  describe("pruneMemory", () => {
    it("prunes without error", () => {
      const pruned = pruneMemory(365);
